...
SKILL_VERSIONS_JSON='["security@9.9.9","context@1.0.0"]' \
bash skills/context/scripts/emit-json.sh > "$WORK/stdout.json" 2>/dev/null
...
if [ "$SKILL" != "security" ]; then
    echo "FAIL: skill='$SKILL' (expected security)"
    exit 1
fi
...
if [ "$VERSIONS" != "security@9.9.9,context@1.0.0" ]; then
