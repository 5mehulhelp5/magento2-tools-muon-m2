...
description:
    ...
    consumer'. Goes beyond magento2-tools:module-create's queue stub. For a new module use
...
This goes beyond the bare queue stub `module-create` emits: it wires all five
...
- **Topic name convention:** `{vendor_lower}.{module_lower}.{entity}.{action}` (all
  ...
  `context/references/naming.md` §9 Queue.
...
**Hard stop if the target module does not exist.** Offer `module-create` and do
...
Follow `context/references/tdd-discipline.md`.
...
`{ctx.docs_root}/message-queues/`. `feature` passes this so a feature
...
```
/message-queue --module=Acme_Orders --topic=acme.orders.order.export \
...
- `context/references/naming.md` — topic/queue/consumer naming conventions.
...
| The bare queue stub | `module-create` |
| After | `review --diff` |
