...
description:
    ...
    form use `magento2-tools:admin-form`; for a new module use `magento2-tools:module-create`.
---
...
not exist, stop and offer `module-create` first — do not scaffold config into a
non-existent module.
...
Follow `context/references/tdd-discipline.md`. Run tests and confirm they fail
...
```
/system-config --module=Acme_Checkout --section=acme_checkout --group=general --field=enable_feature --type=select
/system-config --module=Acme_Catalog --section=acme_catalog --group=api --field=api_key --type=obscure [--docs-root=<path>]
```
...
`{ctx.docs_root}/system-config/`. `feature` passes this so a feature
...
- `context/references/naming.md` — naming conventions (including config path).
...
| Form for config data (not store config) | `admin-form` |
| After | `review --diff` |
