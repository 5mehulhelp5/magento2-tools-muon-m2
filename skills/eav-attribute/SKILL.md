...
description:
    ...
    Produces a Setup/Patch/Data/ class that passes magento2-tools:review. For non-EAV or bulk
    data seeding/migration use magento2-tools:data-migration.
---
...
- **Test-first for the attribute effect.** Write the failing integration test before the patch
  ...
  model's `toOptionArray()`). See `context/references/tdd-discipline.md`.
...
`context/references/tdd-discipline.md` and the integration patterns in
`test-generate/references/integration-patterns.md`. Run it and **confirm it fails for
...
```
/eav-attribute --entity=product --code=acme_color --label="Acme Color" --type=select --module=Acme_Catalog [--docs-root=<path>]
```
...
`{ctx.docs_root}/eav-attributes/`. `feature` passes this so a feature
...
- `context/references/tdd-discipline.md` — shared test-first loop applied in Phase 3A
  (integration patterns: `test-generate/references/integration-patterns.md`).
...
`context/references/placeholder-schema.md`. This skill's templates use the
...
(Near-identical EAV patch templates also exist in `module-create/templates/`
...
- Generated patch passes `review` with zero Critical/High findings.
...
|----------|-------------------------------------------------------------------------------|
| 0        | `context`                                                            |
| (caller) | `feature` Phase 5 — when blueprint declares EAV attributes |
