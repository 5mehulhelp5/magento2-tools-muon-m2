...
- **Assumes the entity exists.** This skill operates on the model / resource-model / `db_schema`
  produced by `module-create`. If the entity model is absent, stop and run
  `module-create` first — do **not** scaffold persistence here.
...
Invoke `context` (vendor, runner, Magento root, version). Then detect the target entity:
- Locate `Model/{Entity}`, `Model/ResourceModel/{Entity}`, and any existing `Api/Data/{Entity}Interface`.
- If the entity **model is absent**, stop: instruct the user to run `module-create` first.
...
- Run `review --diff` on the module. **Gate: zero Critical/High.**
...
```
/webapi [--module=<Vendor>_<Module>] [--entity=<Entity>] [--auth=anonymous|self|acl]
```
...
|-------|-------|
| 0 | `context` |
| (prerequisite) | `module-create` (entity + persistence) |
| (after Phase 3) | `review --diff` |
| (after Phase 3) | `test-generate --types=api` |
| (caller) | `feature` Phase 5 (A* / API tasks) |
| (sibling) | `graphql` (GraphQL surface for the same entity) |
