...
Severities are the shared five-point scale (`context/references/severity.md`). Findings are
...
The consolidated document is `outputKind=audit`, `skill=audit`, written to
`{output_root}/audits/{Vendor}_{Module}-audit-{date}.{json,sarif}` via the shared
`context/scripts/emit-findings.sh` — the same emitter every findings skill uses, so the
