...
- **Validate first.** No release happens if pre-flight validation fails (
  `deploy --validate-only --strict --env=local`). The release MUST use `--validate-only` — never a
...
Invoke `context`. Capture `gh` CLI availability.
...
Run `deploy --validate-only --strict --env=local` against the local environment.
...
```
/release [--version=X.Y.Z] [--no-publish] [--no-github-release] [--dry-run] [--docs-root=<path>] <Vendor>_<Module>
```
...
`{ctx.docs_root}/releases/`. `feature` passes this so a feature run's
...
- `references/changelog-format.md` — commit → category mapping and entry-generation
  procedure; cites `context/references/changelog-format.md` for structure.
...
|-------|--------------------------------------------------------|
| 0     | `context`                                     |
| 2     | `deploy --validate-only --strict --env=local` |
| 6     | external: `gh` CLI                                     |
