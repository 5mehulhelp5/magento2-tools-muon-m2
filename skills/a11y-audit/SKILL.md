...
description:
    ...
    `magento2-tools:frontend`; for general module quality use `magento2-tools:review`.
---
...
- **THEME-AWARE.** Resolve the active theme via `context` (`{ctx.theme}`).
...
Invoke `context`. Capture `{ctx}` including `{ctx.theme}`, `{ctx.tools}`, and
...
1. **Markdown audit report** (LLM deliverable, NOT automated):
   ...
   naming the executing skill: `frontend` for template fixes).
...
```
/magento2-tools:a11y-audit [--module=<Vendor>_<Module>]
    [--theme=<Vendor>/<Theme>]
...
to `{ctx.docs_root}/accessibility/`. Orchestrators such as `feature`
...
| Concern | Skill |
|---------|-------|
| Build accessible frontend templates / components | `frontend` |
| General module quality/architecture review | `review` |
| Context resolution (Phase 0) | `context` |
| Deep static-analysis gate (phpcs, phpstan) | `lint` |
