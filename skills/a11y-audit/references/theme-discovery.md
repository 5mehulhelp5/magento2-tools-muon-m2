...
Cross-references:
- Active theme resolution: `context/references/theme-detection.md`
...
## Using `{ctx.theme}` from context

The `context` skill exposes the active frontend theme as `{ctx.theme}` (field
...
```bash
# Consumer pattern (from context/references/theme-detection.md)
