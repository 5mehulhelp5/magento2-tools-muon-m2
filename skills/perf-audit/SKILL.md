...
description:
    ...
    read-only slow-query inspection in magento2-tools:debug.
---
...
- **JSON + SARIF.** Same shape as `review`.

## Workflow

### Phase 0 — Context Resolution

Invoke `context`. Note presence of Magento CLI, Redis CLI, DB CLI, Blackfire.
...
passes of this skill are dispatched to the read-only `reviewer` agent with a performance/operations
