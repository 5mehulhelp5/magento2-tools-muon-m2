...
description:
    ...
    CVE/secret/EQP security scan use `magento2-tools:security`; to actually version/tag/publish
    a release use `magento2-tools:release`.
---
...
- **REUSE EQP static rules.** Phase 2 delegates EQP coding-standard checks to
  `security`'s EQP scan rather than duplicating its rule list. See
...
Invoke `context`. Capture `{ctx}`. Hard-stop if the target module directory does
not exist.
...
2. **EQP static rules** — delegate to `security`'s Phase 5 (the Magento
   coding-standard / EQP static pass). Do **not** re-implement EQP rules here; incorporate
   the `security` EQP findings into the combined findings list. If
   `security` is not available, skip this sub-check and record a
...
2. **JSON + SARIF** (automated via `${CLAUDE_SKILL_DIR}/scripts/build-findings.sh`). The
   ...
   `security` EQP findings when `EQP_FINDINGS_FILE` is provided (Phase 2.2)
...
to `{ctx.docs_root}/marketplace/`. Orchestrators such as `feature`
...
| Concern | Skill |
|---------|-------|
| Deep CVE / secret / EQP static scan | `security` |
| Version bump, changelog, tag, publish | `release` |
| Context resolution (Phase 0) | `context` |
| Generate MFTF / API tests (if gaps found) | `test-generate` |
