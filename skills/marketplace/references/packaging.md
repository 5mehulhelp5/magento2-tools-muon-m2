...
`marketplace` is read-only and never runs these commands; this document
...
Run these commands from the module root before submission. `marketplace`
...
3. All blocker findings from `marketplace` are resolved.
4. All blocker findings from `security` (EQP static scan) are resolved.
5. Version in `composer.json` matches the Marketplace submission version.
6. `release` has been run to bump version and update `CHANGELOG.md`.
...
- `release` — version bump, changelog, tag, and publish workflow.
