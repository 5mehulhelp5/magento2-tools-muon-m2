...
`marketplace`. For **EQP static code rules** (phpcs Magento2 coding
standard, security patterns), see
`security/references/eqp-rules.md` — those rules are delegated
to `security` rather than duplicated here.
...
- `security` Phase 5 — Marketplace coding-standard checks
- `security/references/eqp-rules.md` — the authoritative EQP rule map

When running `marketplace`, Phase 2 **delegates** to `security`'s
