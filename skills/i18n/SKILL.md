...
Invoke `context`. Capture Magento CLI availability.
...
```
/i18n [--locales=en_US,de_DE,fr_FR] [--machine-translate] [--module=<Vendor>_<Module>] [--docs-root=<path>]
```
...
`{ctx.docs_root}/i18n/`. `feature` passes this so a feature run's
