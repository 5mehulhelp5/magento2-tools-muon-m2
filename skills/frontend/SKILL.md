...
Frontend-specific scaffolding `module-create` doesn't cover.

## Core Rules

- **Theme-aware.** Detect theme from `context` (Luma / Hyva / custom). Hyva
  installs use Alpine.js + Tailwind, NOT Knockout/RequireJS.
- **Breeze-aware.** `context` emits a separate `theme.breeze` object
  ...
  Breeze themes or Breeze widgets — it **routes** them: theme work → `breeze-theme`,
  widget/JS adapter work → `breeze-adapt`. See Phase 1.
...
Invoke `context`. Capture theme detection result.
...
- If `theme.breeze.active` is true (or the user asks for a "Breeze theme" / "Breeze child
  theme") and the operation is `theme` → **stop and route to `breeze-theme`**.
...
- If `theme.breeze.active` is true and the operation is `requirejs-module` or `ko-component`,
  ...
  Luma-fallback scaffold if the user still wants it, but **suggest `breeze-adapt`**
  to add the matching Cash `$.widget` adapter (run `breeze-compat` first to
...
- **Apply the shared module-hygiene baseline (required).** After generating or modifying PHP
  files, run
  `${CLAUDE_PLUGIN_ROOT}/skills/context/scripts/add-license-headers.sh <path> {Vendor}`
...
| Breeze (`theme.breeze.active`) | Cash-based stack; no RequireJS/KO. Route theme → `breeze-theme`, widgets → `breeze-adapt` |
...
|-------|-------|
| 0 | `context` |
| 1 (Breeze theme) | `breeze-theme` — scaffold a Swissup Breeze child theme |
| 1 (Breeze widget/JS) | `breeze-adapt` — build the Breeze Cash adapter; `breeze-compat` to scope it first |
| (after Phase 3) | `review` for the host module |
| (caller) | `feature` Phase 5 (F* tasks) |
