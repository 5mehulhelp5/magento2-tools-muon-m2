...
description:
    ...
    Produces files that pass magento2-tools:review with zero Critical/High findings.
---
...
- **Test-first.** Write the failing test before the form code (red → green) and **watch it fail
  for the right reason**. See `context/references/tdd-discipline.md`.
- **Coding style.** Generated PHP follows PER-CS 3.0 baseline, Magento 2 standard taking
  ...
  `context/references/php-coding-style.md`.
...
Invoke `context`. Capture `{ctx}`. Abort with a clear message if the target module
does not exist (offer `module-create`) or if `{ctx.magento_root}` is unresolved.
...
`context/references/tdd-discipline.md` and
`test-generate/references/integration-patterns.md`.
...
```
/admin-form --module=Acme_Faq --entity=Faq --primary=faq_id --wysiwyg=content --listing=acme_faq_listing [--docs-root=<path>]
```
...
`{ctx.docs_root}/adminhtml-forms/`. `feature` passes this so a feature
...
`context/references/placeholder-schema.md` using the `{Vendor}` / `{Module}` /
...
|-------|-------|
| 0 | `context` |
| upstream | `module-create` — create the module first if absent |
| sibling | `eav-attribute` — add an attribute, then expose it on this form |
| downstream | `test-generate` — fuller unit/integration/MFTF coverage |
| gate | `review` — acceptance gate (zero Critical/High) |
| (caller) | `feature` — the admin-form slice of a feature |
