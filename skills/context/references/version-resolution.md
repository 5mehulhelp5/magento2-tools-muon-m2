...
> Emitting `3.2.0` matches no `2.4.x` range anywhere — every range-matching consumer
> (the BC-break matrix, edition-aware advisory checks) comes back empty. The failure is
...
| `composer.json` constraint uses an alias (e.g. `dev-main as 2.4.7`) | **Not parsed.** `magento_version = null` + a source naming the constraint; consumers must treat null as "matcher did not run", never as clean. See below. |
...
`resolution_source` naming the raw constraint; every consumer must treat that null as
"version-range checks did not run", never as a clean result.
