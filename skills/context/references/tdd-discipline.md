...
The canonical red → green → refactor loop for every builder skill in this pack that emits
**behaviour**. This is the single source of truth; skills point here instead of restating it.
`fix` already applies this loop to defect remediation — the same discipline, now
