...
When a magento2-tools orchestrator may defer to a generic, session-available process skill
...
The orchestrators (`feature`, `fix`, `upgrade`)
...
|---|---|---|
| Safe parallel sub-agent dispatch (isolation, no shared state) | `superpowers:dispatching-parallel-agents` | `feature/references/task-breakdown-guide.md` § Parallel Execution |
...
| Code review | `review` | Domain-specific (ACL, output escaping, DI, Marketplace EQP). A generic review skill misses the Magento rule set. |
