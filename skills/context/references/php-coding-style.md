...
skill in this pack**. Consumed by the builder skills (module-create, graphql,
eav-attribute, data-migration, frontend, fix) and by `review` as a
review lens.
...
| PHPDoc content | Largely unspecified | Magento PHPDoc rules apply — FQCN with leading `\` in `@param`/`@return`/`@throws`, `@api` on `Api/` interfaces, etc. See `module-create/references/phpdoc-rules.md` and `review/references/phpdoc-code-style.md`. |
| Identifier naming | Style-only | Magento naming conventions win — class/method/variable/constant/XML-id/table/config-path naming per `context/references/naming.md`. |
...
`review` reports PER-CS/style deviations at the severity already defined in
