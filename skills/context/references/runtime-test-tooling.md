...
`feature` (Phase 6B smoke), `deploy` (post-deploy smoke),
`fix` (reproduction), and `a11y-audit` (runtime pa11y pass).
Those skills point here; they do not restate these rules.

Related: `context/references/tool-probe.md` (where `tools.curl` and
`tools.headless_browser` come from), `context/references/severity.md` (the severity
...
Drive them through the owning skill's own script (`feature` uses
`${CLAUDE_PLUGIN_ROOT}/skills/feature/scripts/smoke-browser.mjs`, which walks
...
It is **not** a field of the `context` JSON. That cache is keyed on `composer.lock`, so
