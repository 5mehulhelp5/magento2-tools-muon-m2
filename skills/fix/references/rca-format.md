...
These four levels ARE the shared 5-level scale (`context/references/severity.md`)
...
fix — they become separate `/fix` runs.}
...
When dispatched, `explorer` honors the `CLAUDE.md` directive `Explorer model: {tier}`
