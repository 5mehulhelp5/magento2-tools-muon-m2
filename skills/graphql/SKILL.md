...
description:
    ...
    resolvers, batch resolvers, auth checks, and tests. Goes beyond magento2-tools:module-create's
...
- Per-resolver unit tests via `test-generate`
...
- Generate `Test/Unit/Model/Resolver/{Name}Test.php` via `test-generate`.
...
```
/graphql [--module=<Vendor>_<Module>] [--operation=query|mutation] [--auth=customer|admin|anonymous]
```
...
| (after Phase 3) | `test-generate --types=unit` |
| (caller) | `feature` Phase 5 (G* tasks) |
| (sibling) | `webapi` (REST / Web-API surface for the same entity) |
