...
`feature` has four modes. The mode is chosen during Phase 1 from the
...
**Documentation scope (Phase 7A):** full set — per-module docs via `docs`,
...
**Hotfix still invokes `review` after the change** (diff mode). It still
...
| 5 | Execute via `module-create --augment` for the new files; mark each task `[x]` in `plan.md` `## Current State` on completion — SKILL.md Phase 5 *Per-task completion protocol*. |
...
4. Invoke `review --diff` on the touched module(s).
