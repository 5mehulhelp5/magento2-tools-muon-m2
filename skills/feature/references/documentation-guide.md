...
`app/code`, or a module directory — see the **Artifact location** rule in `context/SKILL.md`.
...
Delegate to `docs` for every module created or modified. It extracts the
...
**One artifact home** Core Rule and `context/references/artifact-layout.md`.
...
`docs` in step 1) for the authoritative per-module detail, and covers how a
...
`docs` in step 1, when the module has a user surface) for the authoritative
...
- Re-run `docs` for every module touched this run.
...
| Mode | Phase 7A scope |
|------|----------------|
| `feature` | Full set: per-module docs via `docs`, `spec.md`, developer + user HTML overviews (cross-linked into the per-module docs) with screenshots, API payload examples when a REST/GraphQL surface exists, plus applicable artifacts. |
...
- [ ] `docs` has been run for **every** module created or modified this run, and
