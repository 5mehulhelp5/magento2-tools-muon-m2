...
| Prefix | Type                                                                                   |
|--------|---------------------------------------------------------------------------------------|
| `M`    | Create new module (maps 1:1 to a `module-create` invocation)                 |
...
| `E`    | EAV attribute (maps 1:1 to a `eav-attribute` invocation)                     |
| `G`    | GraphQL surface design (maps 1:1 to a `graphql` invocation)           |
| `F`    | Frontend asset (maps 1:1 to a `frontend` invocation, when present)    |
| `I`    | Wire an extension point onto existing code (maps 1:1 to a `extension-point` invocation — plugin/observer/preference) |
| `C`    | Add admin store configuration (maps 1:1 to a `system-config` invocation)     |
| `L`    | Add a CLI command or cron job (maps 1:1 to a `cli-command` invocation)       |
| `Q`    | Add an async message-queue surface (maps 1:1 to a `message-queue` invocation) |
| `T`    | Write or expand tests (delegates to `test-generate` when present)            |
| `R`    | Review (maps 1:1 to a `review` invocation, `--diff` mode)             |
| `V`    | Validate (quality gate; may delegate to `lint` when present)      |
| `D`    | Deploy (maps 1:1 to a `deploy` invocation)                                   |
