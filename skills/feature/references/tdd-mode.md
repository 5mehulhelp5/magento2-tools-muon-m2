...
line are defined once in `context/references/tdd-discipline.md` — this file only covers
how `feature` applies it.
...
| `hotfix` | Already test-coverage-required; when the change is a defect, it follows the same loop as `fix`. |
...
3. **`T*` becomes a top-up, not the first author.** Because the behaviour tests were written
   ...
   `test-generate` is still used here for that backfill; it is not the primary author of
