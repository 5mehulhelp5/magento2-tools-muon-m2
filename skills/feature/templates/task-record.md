...
and record the concrete failure here (e.g. `system-config — not Skill-invocable: no such
...
- `review` shows no Critical or High findings
...
Depends on: V1
Skill: `deploy` — delegated. `feature` **never** runs `bin/magento`
...
Depends on: D1
Skill: `feature` (uses `scripts/smoke-baseline.sh` + environment probe per
...
Depends on: S1
Skill: `feature` (curl / php-curl driven; per `references/smoke-runner.md` §2)
Model tier (advisory): sonnet
...
Depends on: S1
Skill: `feature` (uses `scripts/smoke-browser.mjs admin-login`)
Model tier (advisory): haiku
Estimate: S
...
Depends on: S3
Skill: `feature` (uses `scripts/smoke-browser.mjs stores-config-walk`)
Model tier (advisory): sonnet
...
Depends on: S3
Skill: `feature` (uses `scripts/smoke-browser.mjs grid`)
Model tier (advisory): haiku
Estimate: S
...
Depends on: S3
Skill: `feature` (uses `scripts/smoke-browser.mjs visit`)
Model tier (advisory): sonnet
Estimate: M
...
Depends on: S1
Skill: `feature` (uses `scripts/smoke-browser.mjs customer-flow`)
Model tier (advisory): sonnet
Estimate: M
...
Skill: `feature` (uses `scripts/smoke-tail-since.sh`)
Model tier (advisory): haiku
Estimate: S
...
Depends on: S8
Skill: `feature` (uses `templates/smoke-run-report.md` + `templates/smoke-findings.md`)
Model tier (advisory): opus
Estimate: S
