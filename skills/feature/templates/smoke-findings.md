...
|----|----------|----------|--------------------|-------------------|--------------|---------|--------------|---------------------|-------|
| F1 | Critical | rest_contract | 1 | 2 | S2 | POST /V1/{vendor}/xyz returns 200 for unauthenticated request | fix | 3 | ACL declaration missing in webapi.xml |
| F2 | Critical | php_exception | 1 | 1 | S8 | `Class Vendor\Xyz\Foo not found` in exception.log | debug → fix | 2 | Class name typo in di.xml |
| F3 | High | frontend | 1 | 3 | S7 | New My Account tab "Vehicles" fails to load — uncaught reference error in vehicle.js | frontend | — | Still open; cap reached |
| F4 | Medium | performance | 1 | 1 | S6 | New admin grid renders in 2.3s (target 2.0s) | perf-audit | n/a | Recorded; not loop-gating |
| F5 | Critical | log_error | 1 | 1 | S8 | `Type Error occurred when creating object: Vendor\Xyz\Plugin\Foo` in system.log ×4 — never reached exception.log | debug → fix | 2 | Constructor arg missing from di.xml; signature `main.CRITICAL: Type Error occurred when creating object: Vendor\Xyz\Plugin\Foo, Too few arguments` |
| F6 | High | report_file | 1 | 1 | S8 | New `var/report/api/N` after POST /V1/... — PHP fatal, no log entry at all | debug → fix | — | Decoded copy in `smoke/raw/S8/reports/` |
