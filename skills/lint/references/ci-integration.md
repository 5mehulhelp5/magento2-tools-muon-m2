...
Running `lint` as an automated CI gate: exit codes, SARIF upload,
...
bash skills/lint/scripts/build-findings.sh > /dev/null
...
- name: Static analysis (changed files only)
  run: |
    ...
    OUTPUT_DIR=".docs/quality" \
    bash skills/lint/scripts/run-analysis.sh \
...
{
  "scripts": {
    "quality": "bash skills/lint/scripts/build-findings.sh",
    "quality:fix": "bash skills/lint/scripts/apply-fixes.sh"
  }
}
```
