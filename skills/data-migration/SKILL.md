...
description:
    ...
    existing data idempotently. Produces idempotent patches that pass magento2-tools:review. For
    adding an EAV attribute use magento2-tools:eav-attribute.
---
...
- **Test-first for the data effect.** Write the failing integration test before the patch body
  ...
  patch exists only to turn that test green. See `context/references/tdd-discipline.md`.
...
`test-generate/templates/test-integration-data-patch.php` and follow
`context/references/tdd-discipline.md`. Run it and **confirm it fails for the right
...
```
/data-migration --type=seed|import|transform [source flags] [--docs-root=<path>]
```
...
`{ctx.docs_root}/migrations/`. `feature` passes this so a feature run's
...
- `context/references/tdd-discipline.md` — shared test-first loop; the Phase 2A test
  reuses `test-generate/templates/test-integration-data-patch.php` as its skeleton.
...
|-------|-------|
| 0 | `context` |
| (after Phase 2) | `review` for the affected module |
