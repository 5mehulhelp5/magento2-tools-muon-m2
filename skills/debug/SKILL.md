...
description:
    ...
    actionable performance findings (N+1, caching) use magento2-tools:perf-audit.
---
...
- **Read-only by default.** This skill does not edit source code. If the user wants
  remediation, redirect to `fix` or `perf-audit`.
...
```
/debug logs --since=1h --pattern="checkout"
```
...
```
/debug trace --event=checkout_submit_all_after
/debug trace --method='Magento\Catalog\Model\Product::save'
/debug trace --class='Magento\Catalog\Api\ProductRepositoryInterface'
```
...
```
/debug di --for='Magento\Catalog\Api\ProductRepositoryInterface'
```
...
the context cache, every probe best-effort).
...
```
/debug <mode> [mode-specific flags]
```
...
`context/references/artifact-layout.md`). It only affects the opt-in `--save`
output: when set, write the saved report under `<path>/debug/`; otherwise default to
`{ctx.docs_root}/debug/`. `feature` passes this so a feature run's
...
- defects → `fix`
- slow-query patterns → `perf-audit`
- DI collisions → `security`
