...
description:
  ...
  coverage falls below a target percentage, or when invoked from magento2-tools:feature
...
- **No source modification.** This skill is purely additive. If a source class is
  ...
  `fix` or a refactor task — do not refactor source.
...
Invoke `context`. Capture vendor, runner, tools (phpunit, node, mftf).
...
```
/test-generate [--types=unit,integration,api,js,mftf] [--target-coverage=80] [--docs-root=<path>] <Vendor>_<Module>
```
...
`{ctx.docs_root}/tests/`. `feature` passes this so a feature run's
...
Templates reuse the test skeletons created in `module-create/templates/`
...
|----------|-----------------------------------------------------------------------------------------------------|
| 0        | `context`                                                                                  |
| (caller) | `feature` Phase 6, `fix` Phase 4, `upgrade` Phase 5 |
