...
description:
    ...
    the Magento admin. Pairs with magento2-tools:admin-form (the edit form); for the form itself use
    magento2-tools:admin-form. Detects edition and flags Commerce-only grid features. Produces files
    that pass magento2-tools:review with zero Critical/High findings.
---
...
- **Test-first.** Write the failing test before the listing code (red → green) and watch it fail for the right reason. See `context/references/tdd-discipline.md`.
- **Coding style.** Generated PHP follows PER-CS 3.0 baseline, Magento 2 standard taking precedence; `--standard=Magento2` PHPCS is the gate. See `context/references/php-coding-style.md`.
...
Invoke `context`. Capture `{ctx}`. Abort with a clear message if the target module does
not exist (offer `module-create`) or if `{ctx.magento_root}` is unresolved.
...
| Input | Default | Notes |
|-------|---------|-------|
| Target module | (ask) | Existing `{Vendor}_{Module}`; offer `module-create` if absent |
...
- Optionally run `review` on the module → zero Critical/High.
...
- Links to `review` and `test-generate`
...
```
/admin-listing --module=Acme_Faq --entity=Faq --primary=faq_id --status --paired-form=acme_faq_faq [--docs-root=<path>]
```
...
under `{ctx.magento_root}`. See the **Artifact location** rule in `context/SKILL.md`.
...
`{ctx.docs_root}/adminhtml-listings/`. `feature` passes this so a
...
- `context/references/tdd-discipline.md` — shared test-first loop (Phase 3A).
...
`context/references/placeholder-schema.md` using the `{Vendor}` / `{ModuleName}` /
...
- Passes `review` with zero Critical/High findings.
...
|-------|-------|
| 0 | `context` |
| upstream | `module-create` — create the module first if absent; defers here for standalone grids |
| sibling | `admin-form` — the edit form this listing's actions column links to |
| downstream | `test-generate` — fuller unit/integration/MFTF coverage |
| gate | `review` — acceptance gate (zero Critical/High) |
| (caller) | `feature` — the admin-listing slice of a feature |
