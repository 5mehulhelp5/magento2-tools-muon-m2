...
and the `docs` output. The set lives **inside the module**:

| Artifact | Scope | Location | Required when |
|----------|-------|----------|---------------|
| `README.md` | Overview | `{module}/README.md` | Always (minimal stub in Step 2; regenerated in full here via `docs`) |
| `CHANGELOG.md` | History scaffold | `{module}/CHANGELOG.md` | Always |
| `docs/technical-reference.md` | Technical | `{module}/docs/technical-reference.md` | Always (delegate to `docs`) |
...
(when a UI/config surface is declared) — is **delegated in full** to `docs` for
...
`docs` owns the section structure, per-surface omission rules, and content
...
jobs, REST/GraphQL routes, and DB schema all come from `docs`'s own extraction.
...
| Mode | Step 6 scope |
|------|--------------|
| Normal create | Full set via one `docs` delegation call: README, CHANGELOG, technical reference, developer guide (when a public surface exists), user guide + screenshots (when a UI surface exists); plus API examples (when REST/GraphQL) and applicable artifacts, written separately by this skill. |
| Quick Create | Reduced: `README.md` + `CHANGELOG.md` + `docs/technical-reference.md` — matches what `docs` always produces for a surface-less module. State that the rest of the full doc set (developer/user guides, API examples) is generated when surfaces are added. |
| `--mode=augment` | Refresh-only: re-run `docs` (refreshes README/CHANGELOG/technical-reference/developer-guide/user-guide) and refresh the API examples for the surfaces the augment added. Never leave a doc describing the pre-augment module. |
...
- [ ] `docs` has been run for the module; `README.md`, `CHANGELOG.md`, and
...
- [ ] If `rest_api` is declared, `docs/api-reference.md` exists (produced by `docs`).
- [ ] If `graphql` is declared, `docs/graphql-reference.md` exists (produced by `docs`).
