...
- `db_schema_whitelist.json` must be present when `db_schema.xml` exists — initialize as `{}`.
  ...
  `docs` owns `README.md` and includes the
...
`module-create` with `--mode=augment` so the skill writes only the new files
...
| EAV product attribute  | `Setup/Patch/Data/Add{Code}Attribute.php`         | `eav-attribute/templates/eav-add-product-attribute-patch.php`  |
| EAV customer attribute | `Setup/Patch/Data/Add{Code}Attribute.php`         | `eav-attribute/templates/eav-add-customer-attribute-patch.php` |
| EAV category attribute | `Setup/Patch/Data/Add{Code}CategoryAttribute.php` | `eav-attribute/templates/eav-add-category-attribute-patch.php` |
