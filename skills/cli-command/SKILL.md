...
description:
    ...
    module use `magento2-tools:module-create` first.
---
...
- **Command name convention:** `{vendor_lower}:{module_lower}:{action}` (all lowercase,
  colon-separated). See `context/references/naming.md`.
...
- **Schedule variants:** use `<schedule>` for a literal cron expression
  ...
  with `system-config` to add the field). Default cron group is `default`.
...
**Hard stop if the target module does not exist.** Offer `module-create` and
...
Follow `context/references/tdd-discipline.md`. Run the 3A tests and confirm
...
```
/cli-command --mode=command --module=Acme_Orders --class=SyncOrdersCommand \
  --name=acme:orders:sync --service=Acme\Orders\Service\OrderSyncer

/cli-command --mode=cron --module=Acme_Orders --class=SyncOrders \
...
`{ctx.docs_root}/cli-commands/`. `feature` passes this so a feature
...
| Cron schedule from admin config | `system-config` |
| After | `review --diff` |
