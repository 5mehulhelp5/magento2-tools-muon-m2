...
description:
    ...
    `magento2-tools:module-create`; for multi-surface work use `magento2-tools:feature`.
---
...
not exist, offer `module-create` first.
...
`{ctx.docs_root}/extension-points/`. `feature` passes this so a feature
...
| (caller) | `feature` Phase 5 — when a blueprint declares interception tasks |
| After | `review --diff` |
