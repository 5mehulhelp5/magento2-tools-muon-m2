...
description:
    ...
    `magento2-tools:module-create`; to review/diagnose existing indexer performance use
...
- **Indexer id convention:** `{vendor_lower}_{module_lower}_{entity}` (all lowercase,
  underscore-separated). See `context/references/naming.md`.
...
Follow `context/references/tdd-discipline.md`. Run the 3A tests and confirm they
...
```
/indexer --module=Acme_Catalog --class=ProductStock \
...
`{ctx.docs_root}/indexers/`. `feature` passes this so a feature run's
...
| After | `review --diff` |
| Diagnose existing indexer perf | `perf-audit` |
